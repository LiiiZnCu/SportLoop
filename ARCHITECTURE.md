...
- 归还检测页把 `loans.before_photo_data_url` 和学生新上传的归还照片传给 `analyze-equipment-damage`，函数调用中国区 MiniMax-M3 后返回检测结论，前端把结论写回 `loans.detect_result`；函数要求两张照片是同一可见面和同一关键区域，乒乓球拍正反面这类照片必须判为不可对比。
