...
- 器材档案由管理员端录入和维护；NFC 机器端负责”入库模式”入库同步以及”NFC 验证模式”验证学生借用，管理员端只接收同步结果，也可以手动修改数量和状态。
- 借用流程：学生手机发起借用 → 获取 6 位验证码 → 前往场馆 NFC 机器输入验证码并贴 NFC 标签 → 验证通过后借用记录变为 “待补借出照片” → 学生上传借出前照片 → 完成借用。
...
- 归还检测页把 `loans.before_photo_data_url` 和学生新上传的归还照片传给 `analyze-equipment-damage`，函数调用中国区 MiniMax-M3 后返回检测结论，前端把结论写回 `loans.detect_result`；函数先扫描每张图的单张可见破损，再要求两张照片是同一可见面和同一关键区域，乒乓球拍正反面这类照片必须判为不可对比。
...
- 学生借用需在场馆 NFC 验证机上完成 NFC 标签验证；管理员端不做 NFC 识别。NFC 芯片由外部机器处理，网页只接收机器同步后的入库数据。
- NFC 机器验证模式：手机上获取验证码 → 机器上输入验证码 → 贴器材 NFC 标签 → UID 匹配后借用生效。
