...
- 器材、场馆、认证、借用、续借、批量申请、留言、工单、机器同步记录和管理员操作记录通过 Supabase REST 接口读写。
...
- `venues` 保存管理员维护的场馆名称、位置、开放时间和开放状态；管理员能读写，匿名用户不能访问。
- 数据库没有器材时学生端显示空状态。
...
- `venues` 这类公开前端会访问的表必须同时有显式 Postgres 授权和 RLS 策略，原因是 Supabase Data API 会先检查授权，再进入行级权限。
