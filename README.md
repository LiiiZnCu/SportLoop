...
单件借还流程：学生先在手机端发起借用并获取 6 位验证码，再到场馆 NFC 机器输入验证码并贴器材标签；验证通过后，学生上传借出前照片，保存后借用才生效。归还时学生上传归还照片，MiniMax 对比借出前照片，只有检测正常时才把“允许机器扫码归还”的标记写回后端。
...
检测逻辑：学生借出成功前先上传“借出前照片”；归还检测页只上传“归还照片”。网页把已保存的借出前照片和归还照片交给 Supabase Edge Function，函数再调用 MiniMax-M3 返回“正常/异常、可信度、是否为目标器材、是否可对比、风险、问题点和说明”。函数会先分别扫描每张图里的单张可见破损，再判断两张图是否可对比。只有两张图都清楚显示目标器材、同一可见面和同一关键区域可对比、可信度达标且没有新增损耗时，才会写入机器归还允许标记；乒乓球拍黑胶面 vs 红胶面这类正反面照片必须判为不可对比并要求重拍，否则显示异常，学生可重新上传或反馈给管理员。
